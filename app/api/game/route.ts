import { identity, validateRequestOrigin, corsResponse, preflight } from '@/lib/request-identity';
...
async function withOrigin(req: Request, handler: (req: Request) => Promise<Response>) {
  let response: Response;
  try { validateRequestOrigin(req); response = await handler(req); }
  catch (error) { response = failure(error, ''); }
  return corsResponse(req, response);
}
export async function GET(req: Request) { return withOrigin(req, handleGet); }
export async function POST(req: Request) { return withOrigin(req, handlePost); }
